<script lang="ts">
	const copyCmd = () => {
		const command = document.getElementById('cmd') as HTMLInputElement;
		command.select();
...
<p class="text-white">
	Please run <input
		type="text"
		class="text-red-600 font-bold rounded-md"
		value="/login token:{data.dcToken}"
		id="cmd"
	/> on Discord
</p>
...
<button
	...
	on:click={copyCmd}>Copy command</button
>
